...
ctr_object* ctr_file_list(ctr_object* myself, ctr_argument* argumentList) {
	...
	while((entry = readdir(d))) {
		...
		if ((strlen(pathValue) + strlen(CTR_DIRSEP) + strlen(entry->d_name)) >= PATH_MAX) {
			continue;
		}
...
ctr_object* ctr_file_stat(ctr_object* myself, ctr_argument* argumentList) {
	...
	ctr_object* size_key = ctr_build_string_from_cstring("size");
	ctr_object* size = ctr_build_number_from_float((double) st.st_size);
	size->info.sticky = 1;
	size_key->info.sticky = 1;
	arg_key.object = size_key;
	arg_val.object = size;
	ctr_map_put(info, &arg_val);
	size->info.sticky = 0;
	...
	atime->info.sticky = 1;
	atime_key->info.sticky = 1;
	arg_key.object = atime_key;
	arg_val.object = atime;
	ctr_map_put(info, &arg_val);
	atime->info.sticky = 0;
	...
	mtime->info.sticky = 1;
	mtime_key->info.sticky = 1;
	arg_key.object = mtime_key;
	arg_val.object = mtime;
	ctr_map_put(info, &arg_val);
	mtime->info.sticky = 0;
	mtime_key->info.sticky = 0;
	ctr_object* uid_key = ctr_build_string_from_cstring("uid");
	ctr_object* uid = ctr_build_number_from_float((double) st.st_uid);
	uid->info.sticky = 1;
	uid_key->info.sticky = 1;
	arg_key.object = uid_key;
	arg_val.object = uid;
	ctr_map_put(info, &arg_val);
	uid->info.sticky = 0;
	uid_key->info.sticky = 0;
	ctr_object* gid_key = ctr_build_string_from_cstring("gid");
	ctr_object* gid = ctr_build_number_from_float((double) st.st_gid);
	gid->info.sticky = 1;
	gid_key->info.sticky = 1;
	arg_key.object = gid_key;
	arg_val.object = gid;
	ctr_map_put(info, &arg_val);
	gid->info.sticky = 0;
	gid_key->info.sticky = 0;
	return info;
}
