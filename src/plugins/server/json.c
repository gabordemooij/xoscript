...
ctr_object* ctr_jsmn_dump( char* data, jsmntok_t** tt ) {
	...
	if (t == NULL) {
		CtrStdFlow = ctr_error(CTR_SRV_JSON_ERR_INVALID, 0);
		return CtrStdNil;
	}
...
void begin_json(){
	...
	ctr_internal_create_func(jsonObject, CTR_STRINGOBJ( CTR_DICT_OBJECT_FROM_SET ), &ctr_json_parse );
	ctr_internal_create_func(jsonObject, CTR_STRINGOBJ( CTR_DICT_JSONIFY ), &ctr_json_jsonify );
	ctr_internal_object_add_property(CtrStdWorld, CTR_STRINGOBJ( CTR_DICT_JSON_OBJECT ), jsonObject, CTR_CATEGORY_PUBLIC_PROPERTY);
}
