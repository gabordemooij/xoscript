...
static strbuf* savechar(strbuf *sb, int idx, int c) {
	if (sb == 0 || idx >= sb->size) {
		sb = sb_get(sb, idx + 1);
	}
	sb->str[idx] = c;
...
static strbuf* savestr(strbuf *sb, const char *str) {
	int len = strlen(str);
	if (sb == 0 || len >= sb->size) {
		sb = sb_get(sb, len + 1);
	}
...
static CGI_varlist* findvar(CGI_varlist *v, const char *varname) {
	if (varname == 0 && v != 0) {
		return v->iter;
	}
	for (; v != 0; v = v->next) {
...
static char* urlencode(const char *in, char *out, const char *keep) {
 ...
 */
static char* scanheader(char *p, char *header[2]) {
...
static strbuf* copyvalue(const char *boundary, FILE *in, const int wantfile, strbuf *value, FILE *out) {
	int c, i, k, matched;
 ...
 */
static CGI_varlist* read_multipart(CGI_varlist *v, const char *template) {
	...
	*/
	...
	for (const char *pt = token[1]; *pt; pt++) {
		if (strchr(RFC_SAFE_BOUNDARY_CHARS, *pt)==NULL) goto cleanup;
	}
